Shader "Azure[Sky]/Static Multi Layer Clouds"
{
	SubShader
	{
		...
		Pass
		{
			...
			// Inputs
			uniform sampler2D   _Azure_SunTexture, _Azure_MoonTexture, _Azure_StarFieldTexture, _Azure_StaticCloudSourceTexture, _Azure_StaticCloudTargetTexture;
			...
			uniform float     _Azure_StaticCloudInterpolator, _Azure_StaticLowCloudSpeed, _Azure_StaticHigCloudSpeed, _Azure_StaticCloudScattering, _Azure_StaticCloudExtinction;
			uniform float     _Azure_StaticCloudDensity, _Azure_StaticCloudOpacity;
			...
			float4 frag (v2f Input) : SV_Target
			{
				...
				float2 lowCloudUv = float2(-atan2(viewDir.z, viewDir.x), -acos(viewDir.y)) / float2(2.0 * PI, PI) + float2(-_Azure_StaticLowCloudSpeed, 0.0);
				float2 higCloudUv = float2(-atan2(viewDir.z, viewDir.x), -acos(viewDir.y)) / float2(2.0 * PI, PI) + float2(-_Azure_StaticHigCloudSpeed, 0.0);
				...
				lowCloudColor = pow(lowCloudColor, _Azure_StaticCloudDensity);
				higCloudColor = pow(higCloudColor, _Azure_StaticCloudDensity);
